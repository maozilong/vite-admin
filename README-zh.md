...
> vite-admin是一个基于Vite 2.x和Vue 2.x的开箱即用、配置简单的后台管理模板，可帮助用户快速搭建基础的后台管理系统。使用了Element-UI组件库，并针对常见的后台管理应用场景，封装了多个组件和页面模板。开发一个实现增删改查的页面只需要配置几行代码即可，大大减少重复工作量。

> 由于采用了Vite 2.x进行构建，vite-admin具有非常快的初始化和热更新速度，在开发阶段能够有效提高生产效率。总之，如果你正在寻找一款快速搭建的Vue.js后台管理模板，并需要具有良好性能的解决方案，vite-admin是值得一试的开源项目。

## 演示
[在线预览](https://maozilong.github.io/vite-admin)
